...
@section('meta_description', 'Jim Corbett Wedding Manage Safari Date Jungle Trail Create Page')
@section('content')
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 mt-4">
        <div>
            <h1> Create Jim Corbett Wedding Event </h1>
            <form action="{{ route('date.jungle.trail.created') }}" method="POST">
                @method('post')
                @csrf
                <div class="row">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select" id="mode" name="mode" aria-label="Select Mode">
                            ...
                            @foreach (getSafariModes() as $mode)
                                <option value="{{ $mode }}" @if ($mode === old('mode')) selected @endif>
                                    {{ $mode }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select" id="zone" name="zone" aria-label="Select Zone">
                            ...
                            @foreach (getSafariZones() as $zone)
                                <option value="{{ $zone }}" @if ($zone === old('zone')) selected @endif>
                                    {{ $zone }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select" id="time" name="time" aria-label="Select Time">
                            ...
                            @foreach (getSafariTimings() as $time)
                                <option value="{{ $time }}" @if ($time === old('time')) selected @endif>
                                    {{ $time }}
                                </option>
                            @endforeach
                        </select>
